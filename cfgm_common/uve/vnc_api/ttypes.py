...
class VncApiStats(object):
  ...
  thrift_spec = (
    ...
    (10, TType.U64, 'response_code', None, None, ), # 10
  )

  def __init__(self, operation_type=None, user=None, useragent=None, remote_ip=None, domain_name=None, project_name=None, object_type=None, response_time_in_usec=None, response_size=None, response_code=None,):
  ...
  def read(self, iprot):
    ...
    while True:
      ...
      if fid == 1:
        ...
        if ftype == TType.U64:
          (length, self.response_code) = iprot.readU64();
          if length < 0: return -1
          read_cnt += length
        else:
          iprot.skip(ftype)
  ...
  def write(self, oprot):
    ...
    if self.response_code is not None:
      annotations = {}
      if oprot.writeFieldBegin('response_code', TType.U64, 10, annotations) < 0: return -1
      if oprot.writeU64(self.response_code) < 0: return -1
      if oprot.writeFieldEnd() < 0: return -1
    if oprot.writeFieldStop() < 0: return -1
    if oprot.writeStructEnd() < 0: return -1
    return 0
  ...
  def log(self):
    ...
    if self.response_size is not None:
      ...
      log_str.write('  ')
    if self.response_code is not None:
      log_str.write('response_code = ')
      log_str.write(str(self.response_code))
      log_str.write('  ')
    return log_str.getvalue()
  ...
  def __sizeof__(self):
    ...
    if self.response_code is not None:
      size += getsizeof(self.response_code)
    return size
...
class VncApiError(sandesh_base.SandeshSystem):
  ...
  def __sizeof__(self):
    size = 0
    if self.api_error_msg is not None:
      size += getsizeof(self.api_error_msg)
    if self.file is not None:
      size += getsizeof(self.file)
    if self.line is not None:
      size += getsizeof(self.line)
...
class VncApiStatsLog(sandesh_base.SandeshObject):
  ...
  def write(self, oprot):
    ...
    if self.api_stats is not None:
      annotations = {}
      annotations['tags'] = '.operation_type,.user,.useragent,.remote_ip,.domain_name,.project_name,.object_type,.response_time_in_usec,.response_size,.response_code'
