...
class ContrailServiceAppliance(contrail.ContrailResource):
    ...
    def handle_create(self):
        ...
        try:
            ...
            raise Exception(_('service-appliance %s could not be created.') % self.name)
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        if not prop_diff.get(self.PHYSICAL_INTERFACE_REFS):
            ref_obj_list = [ref['to'] for ref in obj_0.get_physical_interface_refs() or []]
        else:
            ref_obj_list = []
        if not prop_diff.get(self.PHYSICAL_INTERFACE_REFS_DATA):
