...
class ContrailVirtualNetwork(contrail.ContrailResource):
    ...
    def handle_create(self):
        ...
        try:
            ...
            raise Exception(_('virtual-network %s could not be created.') % self.name)
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        if not prop_diff.get(self.NETWORK_IPAM_REFS):
            ref_obj_list = [ref['to'] for ref in obj_0.get_network_ipam_refs() or []]
        else:
            ref_obj_list = []
        if not prop_diff.get(self.NETWORK_IPAM_REFS_DATA):
        ...
        if not prop_diff.get(self.NETWORK_POLICY_REFS):
            ref_obj_list = [ref['to'] for ref in obj_0.get_network_policy_refs() or []]
        else:
            ref_obj_list = []
        if not prop_diff.get(self.NETWORK_POLICY_REFS_DATA):
