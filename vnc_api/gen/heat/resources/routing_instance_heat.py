...
class ContrailRoutingInstance(contrail.ContrailResource):
    ...
    def handle_create(self):
        ...
        try:
            ...
            raise Exception(_('routing-instance %s could not be created.') % self.name)
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        if not prop_diff.get(self.ROUTE_TARGET_REFS):
            ref_obj_list = [ref['to'] for ref in obj_0.get_route_target_refs() or []]
        else:
            ref_obj_list = []
        if not prop_diff.get(self.ROUTE_TARGET_REFS_DATA):
