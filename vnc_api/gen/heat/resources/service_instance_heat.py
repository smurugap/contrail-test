...
class ContrailServiceInstance(contrail.ContrailResource):
    ...
    def handle_create(self):
        ...
        try:
            ...
            raise Exception(_('service-instance %s could not be created.') % self.name)
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        if not prop_diff.get(self.INSTANCE_IP_REFS):
            ref_obj_list = [ref['to'] for ref in obj_0.get_instance_ip_refs() or []]
        else:
            ref_obj_list = []
        if not prop_diff.get(self.INSTANCE_IP_REFS_DATA):
