...
class ContrailRoutingPolicy(contrail.ContrailResource):
    ...
    def handle_create(self):
        ...
        try:
            ...
            raise Exception(_('routing-policy %s could not be created.') % self.name)
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        if not prop_diff.get(self.SERVICE_INSTANCE_REFS):
            ref_obj_list = [ref['to'] for ref in obj_0.get_service_instance_refs() or []]
        else:
            ref_obj_list = []
        if not prop_diff.get(self.SERVICE_INSTANCE_REFS_DATA):
        ...
        if ref_obj_list or ref_data_list:
        ...
        if not prop_diff.get(self.ROUTING_INSTANCE_REFS):
            ref_obj_list = [ref['to'] for ref in obj_0.get_routing_instance_refs() or []]
        else:
            ref_obj_list = []
        if not prop_diff.get(self.ROUTING_INSTANCE_REFS_DATA):
