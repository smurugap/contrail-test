...
def get_vn_type(vn_name):
    vn_group_list = ['Private_SNAT_VN','Private_VN','Private_LB_VIP_VN','Private_LB_Pool_VN',
                     'SNAT_GW_VN','Public_FIP_VN','Private_SC_MGMT_VN','Private_SC_Left_VN','Private_SC_Right_VN','Private_VSRX_BGP_VN','Private_VSRX_MX_VN']
...
def generate_cidr(tenant_name,vn_type,already_allocated_cidr):
    ...
    ip_group['Private_VSRX_BGP_VN'] = [i for i in xrange(20,21)]
    ip_group['Private_VSRX_MX_VN']  = [i for i in xrange(21,22)]
    ...
    for i in xrange(len(service_instances)):
        ...
        if si_group['name'] in serial_service_chain_si_groups:
           ...
           try:
              ...
              left_vn_count = 1
           standalone_si_count += si_group['count'] * left_vn_count
        ...
        for tenant_name in tenant_name_list:
            ...
            for vn_group_index in xrange(len(tenant_conf['tenant,vn_group_list'])):
                ...
                if re.search('Private_SC_Right_VN|Private_SC_Left_VN',vn_name_pattern):
                   vn_count     = service_instance_vn_count(global_conf,tenant_conf)
                else:
                   vn_count     = vn_info['count']
...
class VM(Base):
    ...
    def retrieve_vm_info(self,*arg,**kwarg):
      try:  
        ...
        for tenant_name in tenant_name_list:
           ...
           instance_info_l = []
           for inst in si_instances:
               dom,t_name,inst_name=inst['fq_name']
               if t_name != tenant_name:
                  continue
               inst_obj  = self.connections.vnc_lib.service_instance_read(id=inst['uuid'])
               inst_name = inst_obj.name
               st_ref    = inst_obj.service_template_refs[0]
               st_obj    = self.connections.vnc_lib.service_template_read(id=st_ref['uuid'])
               st_mode   = st_obj.service_template_properties.get_service_mode()
               st_type   = st_obj.service_template_properties.get_service_type()
               inst_prop = inst_obj.get_service_instance_properties()
               ha_mode   = inst_prop.get_ha_mode()
               inst_lvn  = inst_prop.left_virtual_network
               inst_rvn  = inst_prop.right_virtual_network
               si_vm     = inst_obj.get_virtual_machine_back_refs()[0]
               si_vm_obj = self.connections.vnc_lib.virtual_machine_read(id=si_vm['uuid'])
               vmi_ref   = si_vm_obj.get_virtual_machine_interface_back_refs()
               mgmt_vmi  = None
               for vmi in vmi_ref:
                   dom,tname,vmi_name = vmi['to']
                   if re.search('management',vmi_name):
                      mgmt_vmi = vmi
                      break
               if vmi_obj_info.has_key(mgmt_vmi['uuid']):
                  vmi_obj = vmi_obj_info[mgmt_vmi['uuid']]
               else:
                  vmi_obj = self.connections.vnc_lib.virtual_machine_interface_read(id=mgmt_vmi['uuid'])
               inst_ips  = vmi_obj.get_instance_ip_back_refs()
               ip_obj    = self.connections.vnc_lib.instance_ip_read(id=inst_ips[0]['uuid'])
               mgmt_addr = ip_obj.get_instance_ip_address()
               inst_info = {}
               inst_info['name'] = inst_name
               inst_info['inst_lvn'] = inst_lvn
               inst_info['inst_rvn'] = inst_rvn
               inst_info['inst_mgmt_ip'] = mgmt_addr
               inst_info['inst_ha_mode'] = ha_mode
               inst_info['st_mode']  = st_mode
               inst_info['st_type']  = st_type
               instance_info_l.append(inst_info)
           tenant_info['%s,SC'%(tenant_name)] = instance_info_l
    ...
    def create(self, name, vn_ids, image='ubuntu',userdata=None,flavor="m1.small",zone=None):
        self.fixture = VMFixture(connections=self.connections, vn_ids=vn_ids, vm_name=name, image_name=image,userdata=userdata,zone=zone)
    ...
    def create_vm(self,*arg,**kwarg):
      try:
        ...
        uid = self.create(vm_name,vn_ids,image_name,userdata,flavor,zone)
      except:
        traceback.print_exc(file=sys.stdout)
      return 
...
class Lbaas(Base):
     ...
     def create_lb_vip_associate_fip(self,*arg,**kwarg):
         ...
         vm_intf_id = vm_refs[0]['uuid']
         dom,t_name,vmi_port_id = vm_refs[0]['to']
     ...
     def create_lb_vips_associate_fips(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_name_list):
         fip_pool_indx = 0
         ...
         for tenant_name in tenant_name_list:
             ...
             if attach_fip:
                ...
                kwargs['fip_pool_vn_id'] = vn_info_dict[u'default-domain:%s:%s'%(tenant_name,fip_pool_vn_name)]
             kwargs_list.append(kwargs)
...
class HostAggregate(Base):
      ...
      def create_host_aggregates(self,conn_obj_list,thread_count,global_conf,tenant_conf):
...
class ServiceTemplateConfig(Base):
      ...
      def retrieve_sc_info(self,*arg,**kwarg):
          ...
          for tenant_name in tenant_name_list:
             ...
             service_chain_info[tenant_name] = vn_pairs_list
      ...
      def create_service_instance(self,*arg,**kwarg):
          ...
          fixture = SvcInstanceFixture(
                ...
                left_vn_name=left_vn_name, right_vn_name=right_vn_name, do_verify=False, max_inst=max_inst, static_route=static_route,az_name=az_name,host_name=host_name,ha_mode=ha_mode)
          fixture.setUp()
      ...
      def create_service_instances(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_name_list):
          ...
          for tenant_name in tenant_name_list:
            ...
            for si in service_instances:
                ...
                for si_indx in xrange(si_count):
                    ...
                    kwargs['az_name']               = si['AZ']
                    kwargs['host_name']             = si['host']
                    ...
                    for indx in xrange(vn_count):
                      ...
                      for interface_type in ['management','left','right']:
                        ...
                        if interface_type in ['left','right'] :
                           if vn_name in ['None',None]:
                              vn_name = None
                           else:
                              vn_name = "default-domain:%s:%s"%(tenant_name,generate_vn_name(global_conf,tenant_conf,tenant_indx,vn_name,vn_index))
                        else:
          ...
          for si in si_single:
              ...
              dst_port = "any"
              policy_rules = []
              for if_vn_dict in si['if_vn_dict']:
                  ...
                  rule = {
                           ...
                           'src_ports': '%s'%src_port, 'dst_ports': '%s'%dst_port,
                           'source_network': '%s'%src_nw, 'dest_network': '%s'%dst_nw,
                           'action_list':{'apply_service':['default-domain:%s:%s'%(tenant_name,service_name)]}
                       }
          ...
          for indx,si_l in enumerate(serial_chain_info_list):
              ...
              for si in si_l:
                  service_name = si['si_name']
                  si_name_l.append('default-domain:%s:%s'%(tenant_name,service_name))
              service_name = si['si_name']
              policy_name  = "serial_sc.%d.pol"%indx
              if_vn_dict   = si['if_vn_dict']
              if isinstance(if_vn_dict,list):
                 if_vn_dict = if_vn_dict[0]
              src_nw = if_vn_dict['left']
              dst_nw = if_vn_dict['right']
          ...
          for indx,si_l in enumerate(parallel_chain_info_list):
              ...
              for indxl,si in enumerate(si_l):
                  service_name = si['si_name']
                  si_name_l = 'default-domain:%s:%s'%(tenant_name,service_name)
          ...
          for si in si_single:
            ...
            for vn_dict in if_vn_dict:
              kwargs = {}
              kwargs['tenant_name'] = si['tenant_name']
      ...
      def delete_service_instances(self,conn_obj_list,thread_count,global_conf,tenant_conf,tenant_name_list):
          kwargs_list = []
          for tenant_name in tenant_name_list:
               kwargs = {}
               kwargs['tenant_name'] = tenant_name
